...
module.exports = {
    ...
    async execute (client, message, args) {
        ...
        if(porcentaje <= 23){
            description = `** ${porcentaje}% | ** Ni siquiera debieron conocerse, los 2 no son compatibles y son muy diferentes.`
            emoji = 'https://images.emojiterra.com/twitter/v13.0/512px/1f645-2640.png'
        }

        if(porcentaje > 23 && porcentaje <= 47){
            description = `** ${porcentaje}% | ** Podrian ser amigos, pero no veo un futuro mayor entre ellos, tienen algunos gustos similares.`
            emoji = 'https://images.emojiterra.com/twitter/v13.0/512px/1f91d.png'
        }

        if(porcentaje > 47 && porcentaje <= 80){
            description = `** ${porcentaje}% | ** Podrian ser una pareja, los gustos son casi iguales y saben mucho uno del otro.`
            emoji = 'https://images.emojiterra.com/twitter/v13.0/512px/1f440.png'
        }

        if(porcentaje > 80){
            description = `** ${porcentaje}% | ** Serian una excelente pareja, pueden llegar a casarse algun dia, los gustos son iguales y se llevan muy bien.`
        ...
        message.channel.send(description, attachment)
    }
}